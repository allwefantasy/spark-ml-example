...
object RegressionExample {
   def main(args:Array[String]):Unit = {
     ...
     //lr.setSolver("auto") 设置优化函数(求解损失函数)，如果为Normal
     // 则采用Normal Equation (正规化方程)
     // 如果为auto,则判定如果     elasticNetParam==0 && numFeatures < 4096
     // 则也选用Normal Equation (正规化方程)
     // 如果选用正规化方程，会默认使用 L2 正则化向，
     // 并且采用WeightedLeastSquares
     // 否则L-BFGS(Limited-Memory BFGS)是BFGS算法在受限内存时的一种近似算法，
     // 而BFGS是数学优化中一种无约束最优化算法
